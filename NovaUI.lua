...
local NovaUI = isStudio and script.Parent:WaitForChild("Nova UI") or game:GetObjects("rbxassetid://86467455075715")[1]

local function nearColor(color, target, tolerance)
...
local function ApplyDarkRefresh(root)
	for _, instance in ipairs(root:GetDescendants()) do
		if instance:IsA("UIStroke") then
			instance.Color = nearColor(instance.Color, Color3.fromRGB(87, 84, 104), 20) and UITheme.StrokeHover or UITheme.Stroke
			instance.Thickness = math.max(instance.Thickness, 1)
		elseif instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
			if nearColor(instance.TextColor3, Color3.fromRGB(200, 200, 200), 70) or nearColor(instance.TextColor3, Color3.fromRGB(255, 255, 255), 35) then
				local name = string.lower(instance.Name)
				instance.TextColor3 = (name == "desc" or name == "subtitle" or name == "user") and UITheme.TextMuted or UITheme.Text
			end
			if instance:IsA("TextBox") then
				instance.PlaceholderColor3 = UITheme.TextSubtle
			end
		elseif instance:IsA("ImageLabel") or instance:IsA("ImageButton") then
			if nearColor(instance.ImageColor3, Color3.fromRGB(255, 255, 255), 35) or nearColor(instance.ImageColor3, Color3.fromRGB(195, 195, 195), 45) then
				instance.ImageColor3 = UITheme.TextMuted
			end
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Nova:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
NovaUI.SmartWindow.Visible = false
NovaUI.Notifications.Template.Visible = false
NovaUI.DisplayOrder = 1000000000

local Main : Frame = NovaUI.SmartWindow
local Dragger = Main.Drag
local dragBar = NovaUI.Drag
...
-- 	for FlagName, Flag in pairs(Nova.Flags) do
...
-- 		else
-- 			notified = true
-- 			Nova:Notification({Title = "Config Error", Content = "Nova was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		Nova:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using Nova Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		Nova:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using Nova Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(Nova.Flags) do
...
function Nova:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function Nova:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "Nova UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Nova Interface Suite" and Release or "Nova UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(NovaUI.MobileSupport, NovaUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "Nova/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Nova/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".Nova") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".Nova"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".Nova", FoundKey)
						end
						Nova:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		function HomeTab:Activate()
			...
			for _, OtherTabButton in ipairs(Navigation.Tabs:GetChildren()) do
				if OtherTabButton.Name ~= "InActive Template" and OtherTabButton.ClassName == "Frame" and OtherTabButton ~= HomeTabButton then
					tween(OtherTabButton.ImageLabel, {ImageColor3 = UITheme.TextMuted})
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Nova Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
						TweenService:Create(Button.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
						Button.Title.Text = "Callback Error"
						print("Nova Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
						TweenService:Create(Button.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
					else
						tween(Button.UIStroke, {Color = UITheme.StrokeActive})
						wait(0.2)
						if ButtonV.Hover then
							tween(Button.UIStroke, {Color = UITheme.StrokeHover})
						else
							tween(Button.UIStroke, {Color = UITheme.Stroke})
						end
					end
				end)
			...
			function Section:CreateSlider(SliderSettings, Flag)
				...
				Slider["MouseEnter"]:Connect(function()
					tween(Slider.UIStroke, {Color = UITheme.StrokeHover})
				end)

				Slider["MouseLeave"]:Connect(function()
					tween(Slider.UIStroke, {Color = UITheme.Stroke})
				end)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
									TweenService:Create(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
									Slider.Title.Text = "Callback Error"
									print("Nova Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
									TweenService:Create(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
								end

								SliderSettings.CurrentValue = NewValue
								SliderV.CurrentValue = SliderSettings.CurrentValue
								-- Nova.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				local function Set(NewVal, bleh)
					...
					if not Success then
						...
						Slider.Title.Text = SliderSettings.Name
						TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.SurfaceAlt}):Play()
						TweenService:Create(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
					end
				...
				if Flag then
					Nova.Options[Flag] = SliderV
				end

				NovaUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = Nova.ThemeGradient
					Slider.Main.UIStroke.color.Color = Nova.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				local function Set(bool)
					if bool then
						...
						Toggle.toggle.UIStroke.Color = UITheme.ToggleOff

						tween(Toggle.toggle, {BackgroundTransparency = 1})

						tween(Toggle.toggle.val, {BackgroundColor3 = UITheme.ToggleOff, Position = UDim2.new(0,5,0.5,0), BackgroundTransparency = 0})
					end
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
						TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
						Toggle.Title.Text = "Callback Error"
						print("Nova Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
						TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
					end
				end)

				Toggle["MouseEnter"]:Connect(function()
					tween(Toggle.UIStroke, {Color = UITheme.StrokeHover})
				end)

				Toggle["MouseLeave"]:Connect(function()
					tween(Toggle.UIStroke, {Color = UITheme.Stroke})
				end)
				...
				NovaUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = Nova.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = Nova.ThemeGradient
				end)

				if Flag then
					Nova.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
				...
				Bind["MouseEnter"]:Connect(function()
					tween(Bind.UIStroke, {Color = UITheme.StrokeHover})
				end)

				Bind["MouseLeave"]:Connect(function()
					tween(Bind.UIStroke, {Color = UITheme.Stroke})
				end)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								TweenService:Create(Bind, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
								...
								print("Nova Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
								TweenService:Create(Bind, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								TweenService:Create(Bind, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
				...
				if Flag then
					Nova.Options[Flag] = BindV
				end

				-- Nova.Flags[BindSettings.Flag] = BindSettings
			...
			function Section:CreateInput(InputSettings, Flag)
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								TweenService:Create(Input, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
								TweenService:Create(Input.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
								Input.Title.Text = "Callback Error"
								print("Nova Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
								TweenService:Create(Input, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								TweenService:Create(Input, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
				...
				Input["MouseEnter"]:Connect(function()
					tween(Input.UIStroke, {Color = UITheme.StrokeHover})
				end)

				Input["MouseLeave"]:Connect(function()
					tween(Input.UIStroke, {Color = UITheme.Stroke})
				end)
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						TweenService:Create(Dropdown, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
						TweenService:Create(Dropdown.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
						Dropdown.Title.Text = "Callback Error"
						print("Nova Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Dropdown, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
				...
				local function ActivateColorSingle(name)
					...
					Toggle()
					tween(Dropdown.List[name], {BackgroundTransparency = 0.95, TextColor3 = UITheme.Text})
				end
					...
					for i,v in pairs(DropdownSettings.Options) do
							...
							if DropdownSettings.MultipleOptions then
								if table.find(DropdownSettings.CurrentOption, v) then
									...
									if not optionhover then
										tween(Option, {TextColor3 = UITheme.TextSubtle})
									end
									tween(Option, {BackgroundTransparency = 0.98})
								else
									table.insert(DropdownSettings.CurrentOption, v)
									DropdownV.CurrentOption = DropdownSettings.CurrentOption
									tween(Option, {TextColor3 = UITheme.Text, BackgroundTransparency = 0.95})
								end
						...
						Option.MouseEnter:Connect(function()
							...
							if Option.BackgroundTransparency == 0.95 then
								...
								tween(Option, {TextColor3 = UITheme.Text})
							end
						end)
				...
				Dropdown["MouseEnter"]:Connect(function()
					tween(Dropdown.UIStroke, {Color = UITheme.StrokeHover})
				end)

				Dropdown["MouseLeave"]:Connect(function()
					tween(Dropdown.UIStroke, {Color = UITheme.Stroke})
				end)
				...
				if type(bleh) == "string" then 
					tween(Dropdown.List[bleh], {TextColor3 = UITheme.Text, BackgroundTransparency = 0.95})
				else
					for i,v in pairs(bleh) do
						tween(Dropdown.List[v], {TextColor3 = UITheme.Text, BackgroundTransparency = 0.95})
					end
				end
				...
				if DropdownSettings.MultipleOptions then
					...
					for _, name in pairs(DropdownSettings.CurrentOption) do
						tween(Dropdown.List[name], {TextColor3 = UITheme.TextMuted, BackgroundTransparency = 0.95})
					end
				else
					Dropdown.Selected.PlaceholderText = DropdownSettings.CurrentOption[1] or "None"
				end
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					for _, Option in pairs(Dropdown.List:GetChildren()) do
						if Option.ClassName == "TextLabel" then
							tween(Option, {TextColor3 = UITheme.TextSubtle, BackgroundTransparency = 0.98})
						end
					end
					tween(Dropdown.List[bleh], {TextColor3 = UITheme.Text, BackgroundTransparency = 0.95})
					...
					-- Nova.Flags[DropdownSettings.Flag] = DropdownSettings
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				ColorPicker["MouseEnter"]:Connect(function()
					tween(ColorPicker.UIStroke, {Color = UITheme.StrokeHover})
				end)
				ColorPicker["MouseLeave"]:Connect(function()
					tween(ColorPicker.UIStroke, {Color = UITheme.Stroke})
				end)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						TweenService:Create(ColorPicker, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Error}):Play()
						...
						print("Nova Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
						TweenService:Create(ColorPicker, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(ColorPicker, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = UITheme.Surface}):Play()
				...
				if Flag then
					Nova.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = Nova:SaveConfig(inputPath)
					if not success then
						Nova:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = Nova:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = Nova:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Nova:LoadConfig(selectedConfig)
					if not success then
						Nova:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Nova:SaveConfig(selectedConfig)
					if not success then
						Nova:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(Nova.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(Nova.Folder .. "/settings/autoload.txt")
			...
			if isfile(Nova.Folder .. "/settings/autoload.txt") then
				local name = readfile(Nova.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if Nova.Options[Flag] then
						Nova.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if Nova.Options[Flag] and data.value then
						Nova.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if Nova.Options[Flag] and data.text and type(data.text) == "string" then
						Nova.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if Nova.Options[Flag] and data.value then
						Nova.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if Nova.Options[Flag] and data.color then
						Nova.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "NovaInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "NovaInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "NovaInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						Nova.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						NovaUI.ThemeRemote.Value = not NovaUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						Nova.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						NovaUI.ThemeRemote.Value = not NovaUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						Nova.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						NovaUI.ThemeRemote.Value = not NovaUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				Nova.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				Nova.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function Nova:SaveConfig(Path)
			...
			local fullPath = Nova.Folder .. "/settings/" .. Path .. ".Nova"

			local data = {
				objects = {}
			}

			for flag, option in next, Nova.Options do
		...
		function Nova:LoadConfig(Path)
			...
			local file = Nova.Folder .. "/settings/" .. Path .. ".Nova"
		...
		function Nova:LoadAutoloadConfig()
			if isfile(Nova.Folder .. "/settings/autoload.txt") then
				...
				local success, err = Nova:LoadConfig(name)
				if not success then
					return Nova:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function Nova:RefreshConfigList()
			...
			local list = listfiles(Nova.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".Nova" then
					local pos = file:find(".Nova", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			NovaUI.MobileSupport.Visible = true
		end
	end)
	...
	Main.Controls.Close["MouseLeave"]:Connect(function()
		tween(Main.Controls.Close.ImageLabel, {ImageColor3 = UITheme.TextSubtle})
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			NovaUI.MobileSupport.Visible = false
	...
	Main.Controls.ToggleSize["MouseLeave"]:Connect(function()
		tween(Main.Controls.ToggleSize.ImageLabel, {ImageColor3 = UITheme.TextSubtle})
	end)
	...
	Main.Controls.Theme["MouseLeave"]:Connect(function()
		tween(Main.Controls.Theme.ImageLabel, {ImageColor3 = UITheme.TextSubtle})
	end)	


	NovaUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		NovaUI.MobileSupport.Visible = false
...
if (getgenv and not getgenv().ConfirmNova) or (not getgenv) then
	Nova:Notification({ 
    	Title = "Nova Is Deprecated",
    	Icon = "warning",
    	ImageSource = "Material",
    	Content = "If you are not the script developer, ignore this message. \n\n The Nova Interface Library Is Deprecated And Not Recommended to Use. A New Library Is Available at nebulasoftworks.xyz/starlight. If you insist on using Nova, set the getgenv().ConfirmNova variable to true. "
	})
end

if isStudio then
	local Window = Nova:CreateWindow({
		Name = "Nebula Client - Nova Hub | Blade Ball",
		Subtitle = "by Nebula Softworks",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Nebula Client (Nova Hub)",
	...
	Nova:Notification({ 
		...
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the Nova Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Nova was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Nova Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Nova Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "Nova's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = Nova:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the Nova Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Nova was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Nova Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Nova Today at " .. website})
